...
/* @var $items \common\models\ItemVendaForm[] */

$exemplares = ArrayHelper::map(
...
<div class="bg-white p-6 rounded-lg shadow-sm">
    ...
    <?= $form->field($model, 'data_venda')
        ->textInput(['value' => date('Y-m-d'), 'disabled' => true]) ?>
        ...
        <tbody class="container-items">
            <?php foreach ($items as $i => $item): ?>
                <tr class="item border-t">
                    <td>
                        <?= $form->field($item, "[{$i}]exemplar_id")
                            ->dropDownList($exemplares, ['prompt' => 'Selecione'])->label(false) ?>
                    </td>
                    <td>
